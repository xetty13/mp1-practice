...
int* quicksort_memory(ULONGLONG* size, int n1, int n2)
{
    int n = n2 - n1 + 1, i;
    int *newid = (int*)malloc(n * sizeof(int));
    ...
    quicksort(copy_size, n1, n2, newid);
    free(copy_size);
    return newid;
}
...
void main()
{
    ...
    while (1) {
        ...
        begin = clock();
            ...
            case 5:
                scanf("%c", &e);
                newId = quicksort_memory(filesize, fileId[0], count_files - 1);
                end = clock();
                break;
            case 6:
                scanf("%c", &e);
