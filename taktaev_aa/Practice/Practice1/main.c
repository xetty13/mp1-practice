...
void main() 
{
	...
	{
		printf("Одно из введенных значений плотности меньше нуля. Повторите ввод данных");
		return;
	}
	VZad = w * h * 0.5;
	VBok = 2.0f * h * d * 1.5;
	VKrish = 2.0f * w * d * 1.5;
	VDver = w * h * 1;
	VPolk = (w - 1.5 * 2) * d * floor(h / 40) * 1.5; //выражение в скобках показывает, что полки находятся между двумя боковинами шкафа. 1.5 см - это предположительная толщина полки(в задании она не дана)
