...
int process(int n, int* needed, int cur, int playing) {
	// выдел€ем пам€ть под введенное число
...
int* gen(int n) { // генерирует случайное число-массив заданной длинны
	...
	do
		shuffle(digits);
	while (digits[0] == 0);
	// чтобы вернуть результат, создаем указатель на массив, выдел€ем под него пам€ть
...
int introduction() {//¬ступление
	...
	while (true) { //Ѕесконечный цикл
		 // считываем длину числа
		scanf("%d", &n);
		// провер€ем на корректность
		if (n >= 2 && n <= 5) {
			printf("\n");
